...
- **Next:** Day 6 — thin FastAPI app (`app/main.py`: `POST /run`, `GET /results`) + Streamlit UI (`ui/streamlit_app.py`: interactive loop view + results explorer) wired locally, plus the committed read-only `data/demo_results.db` and `reports/output/demo/` set for the zero-cost public demo.
