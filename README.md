...
Submit a task in the UI to watch v1 → feedback → v2 with metrics; the **Results
explorer** tab browses stored experiments and renders the four charts. Endpoints:
`GET /health`, `POST /run` (`{"prompt": "...", "do_judge": false}`), `GET /results`.
...
**Test suite:** 88 tests pass, `ruff` clean.
...
| Test suite (88 tests) | offline (model calls stubbed) | **$0** |
